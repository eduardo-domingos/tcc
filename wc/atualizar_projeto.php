<?php
    ...
    if(isset($_POST['Atualizar'])){
        ...
        $id_usuario = htmlEntities($_SESSION['id_usuario']);
        $id_projeto = htmlEntities($_POST['id_projeto']);
        ...
        if(!empty($video)){
            ...
            if($youtube[1]=="youtube"){
                ...
                echo ('<script>window.alert("URL inválida!");window.location="projetos.php";</script>');
            }
        }
        
        $sql = 'UPDATE projetos SET titulo = "'.$titulo.'", equipe = "'.$equipe.'", resumo = "'.$resumo.'", caracteristicas = "'.$caracteristicas.'", localidade = "'.$localidade.'", data_inicio = "'.$data.'", video = "'.$etec.'", imagem = "'.$nome_imagem.'", valor = "'.$valor.'" WHERE id_usuario = '.$_SESSION['id_usuario'].' AND id_projeto = '.$id_projeto.';';
        #echo($sql);
        ...
        if($atualizar){
            echo('<script>window.alert("Projeto atualizado com sucesso!");window.location="projetos.php";</script>');
            if(!empty($imagem['name'])){
                move_uploaded_file($imagem["tmp_name"], $caminho_imagem);
            }
        }else{
            echo('<script>window.alert("Falha ao atualizar projeto!");window.location="projetos.php";</script>');
        }
